...
class tom_TodoViewModel: ViewModel() {

    val todTodoList:LiveData<MutableList<tom_Todo>>
    private val todoRepository: tom_TodoRepository =tom_TodoRepository.get()
    ...
    fun insert(dto:tom_Todo)=viewModelScope.launch(Dispatchers.IO){
        todoRepository.insert(dto)
    }

    fun update(dto:tom_Todo)=viewModelScope.launch(Dispatchers.IO){
        todoRepository.update(dto)
    }

    fun delete(dto:tom_Todo)=viewModelScope.launch(Dispatchers.IO){
