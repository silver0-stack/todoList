...
class som_TodoViewModel: ViewModel() {

    val todTodoList:LiveData<MutableList<som_Todo>>
    private val todoRepository: som_TodoRepository = som_TodoRepository.get()
    ...
    fun insert(dto: som_Todo)=viewModelScope.launch(Dispatchers.IO){
        todoRepository.insert(dto)
    }

    fun update(dto: som_Todo)=viewModelScope.launch(Dispatchers.IO){
        todoRepository.update(dto)
    }

    fun delete(dto: som_Todo)=viewModelScope.launch(Dispatchers.IO){
